...
class GetPacketHeader : public Module {
private:
...
public:
  GetPacketHeader()
      : Module(ModuleType::x86_Tofino_GetPacketHeader, TargetType::x86_Tofino,
               "GetPacketHeader") {}

  GetPacketHeader(BDD::BDDNode_ptr node, klee::ref<klee::Expr> _p_addr,
                  klee::ref<klee::Expr> _chunk_addr,
                  klee::ref<klee::Expr> _chunk, klee::ref<klee::Expr> _length)
      : Module(ModuleType::x86_Tofino_GetPacketHeader, TargetType::x86_Tofino,
               "GetPacketHeader", node),
...
private:
  processing_result_t process_call(const ExecutionPlan &ep,
    ...
    if (call.function_name == symbex::FN_BORROW_CHUNK) {
      ...
      auto new_module = std::make_shared<GetPacketHeader>(
...
public:
  ...
  virtual Module_ptr clone() const override {
    auto cloned = new GetPacketHeader(node, p_addr, chunk_addr, chunk, length);
  ...
  virtual bool equals(const Module *other) const override {
    ...
    auto other_cast = static_cast<const GetPacketHeader *>(other);
