...
public:
  ...
  MapGet(BDD::Node_ptr node, obj_addr_t _map_addr, klee::ref<klee::Expr> _key,
         ...
         const BDD::symbol_t &_map_has_this_key)
      : x86Module(ModuleType::x86_MapGet, "MapGet", node), map_addr(_map_addr),
        key(_key), value_out(_value_out), success(_success),
...
private:
  processing_result_t process(const ExecutionPlan &ep,
    ...
    if (call.function_name == symbex::FN_MAP_GET) {
      ...
      auto new_module = std::make_shared<MapGet>(
          node, _map_addr, _key, _value_out, _success, _map_has_this_key);
...
public:
  ...
  virtual Module_ptr clone() const override {
    auto cloned =
        new MapGet(node, map_addr, key, value_out, success, map_has_this_key);
