...
public:
  ByteArray(std::string _label, obj_addr_t _addr) : Variable(_label, 0) {
    addr = _addr;
    is_array = true;
  }
  ...
  ByteArray(std::string _label, klee::ref<klee::Expr> _expr, obj_addr_t _addr)
  ...
  ByteArray(std::string _label, klee::ref<klee::Expr> _size,
            klee::ref<klee::Expr> _expr, obj_addr_t _addr)
