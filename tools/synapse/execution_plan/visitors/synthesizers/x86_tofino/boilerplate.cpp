...
struct cpu_hdr_t {
	...
	uint16_t get_cpu_out_port() const {
		return ntohs(out_port) & 0b111111111;
	}
...
void pretty_print_pkt(struct pkt_hdr_t *pkt_hdr) {
	printf("###[ CPU ]###\n");
	printf("  code path  %u\n", ntohs(pkt_hdr->cpu_hdr.code_path));
	printf("  out port   %u\n", ntohs(pkt_hdr->cpu_hdr.out_port));
...
private:
	struct field_t {
		...
		field_t(const std::string &_name, bf_rt_id_t _id)
			: name(_name), id(_id) {}
	};
...
public:
	SynapseTable(const std::string _table_name,
		...
		for (auto param_field_name : _param_fields_names) {
			bf_rt_id_t field_id;
			init_data_with_action(param_field_name, populate_action_id,
								  &field_id);
	...
	static std::unique_ptr<SynapseTable> build(
		const std::string _table_name,
		const std::vector<std::string> _key_fields_names,
		const std::vector<std::string> _param_fields_names) {
		return std::unique_ptr<SynapseTable>(new SynapseTable(
			_table_name, _key_fields_names, _param_fields_names));
	}
