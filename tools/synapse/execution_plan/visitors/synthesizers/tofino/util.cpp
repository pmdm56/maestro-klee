...
std::vector<key_var_t> get_key_vars(Ingress &ingress, klee::ref<klee::Expr> key,
  ...
  while (offset_bits < size_bits) {
    ...
    for (const auto &meta : meta) {
      ...
      // Careful here, don't go find the size of this by meta_varq.var->get_size_bits().
      // That method gives the P4 size. That may differ from the expression size, 
      // for example in the case of ports (16 bits vs 9 bits).
