...
dchain_config_t get_dchain_config(const BDD::BDD &bdd, obj_addr_t dchain_addr) {
  ...
  for (auto init_node : init_nodes) {
    auto call_node = BDD::cast_node<BDD::Call>(init_node);
...
bits_t get_key_size(const BDD::BDD &bdd, obj_addr_t addr) {
  ...
  for (auto node : nodes) {
    ...
    if (call.function_name == symbex::FN_MAP_GET ||
    ...
    else if (call.function_name == symbex::FN_SKETCH_COMPUTE_HASHES) {
      ...
      assert(!call.args[symbex::FN_SKETCH_ARG_KEY].in.isNull());
      auto _key = call.args[symbex::FN_SKETCH_ARG_KEY].in;
  ...
  Log::err() << "Node not found but required to find key size..\n";
  exit(1);
}
...
map_config_t get_map_config(const BDD::BDD &bdd, obj_addr_t map_addr) {
  ...
  for (auto init_node : init_nodes) {
    ...
    assert(call.args[symbex::FN_MAP_ARG_KEY_EQUAL].fn_ptr_name.first);
    assert(call.args[symbex::FN_MAP_ARG_KEY_HASH].fn_ptr_name.first);
    assert(!call.args[symbex::FN_MAP_ARG_MAP_OUT].out.isNull());

    auto capacity = call.args[symbex::FN_MAP_ARG_CAPACITY].expr;
    auto keq = call.args[symbex::FN_MAP_ARG_KEY_EQUAL].fn_ptr_name.second;
    auto khash = call.args[symbex::FN_MAP_ARG_KEY_HASH].fn_ptr_name.second;
    auto map_out = call.args[symbex::FN_MAP_ARG_MAP_OUT].out;
  ...
  Log::err() << "Map configuration not found.\n";
  exit(1);
}
...
vector_config_t get_vector_config(const BDD::BDD &bdd, obj_addr_t vector_addr) {
  ...
  for (auto init_node : init_nodes) {
    ...
    assert(call.args[symbex::FN_VECTOR_ARG_INIT_ELEM].fn_ptr_name.first);
    assert(!call.args[symbex::FN_VECTOR_ARG_VECTOR_OUT].out.isNull());

    auto capacity = call.args[symbex::FN_VECTOR_ARG_CAPACITY].expr;
    auto elem_size = call.args[symbex::FN_VECTOR_ARG_ELEM_SIZE].expr;
    auto init_elem =
        call.args[symbex::FN_VECTOR_ARG_INIT_ELEM].fn_ptr_name.second;
    auto vector_out = call.args[symbex::FN_VECTOR_ARG_VECTOR_OUT].out;
  ...
  Log::err() << "Vector configuration not found.\n";
  exit(1);
}
...
sketch_config_t get_sketch_config(const BDD::BDD &bdd, obj_addr_t sketch_addr) {
  ...
  for (auto init_node : init_nodes) {
    ...
    assert(!call.args[symbex::FN_SKETCH_ARG_CAPACITY].expr.isNull());
    assert(!call.args[symbex::FN_SKETCH_ARG_THRESHOLD].expr.isNull());
    assert(call.args[symbex::FN_SKETCH_ARG_KEY_HASH].fn_ptr_name.first);
    assert(!call.args[symbex::FN_SKETCH_ARG_SKETCH_OUT].out.isNull());

    auto capacity = call.args[symbex::FN_SKETCH_ARG_CAPACITY].expr;
    auto threshold = call.args[symbex::FN_SKETCH_ARG_THRESHOLD].expr;
    auto khash = call.args[symbex::FN_SKETCH_ARG_KEY_HASH].fn_ptr_name.second;
    auto sketch_out = call.args[symbex::FN_SKETCH_ARG_SKETCH_OUT].out;
  ...
  Log::err() << "Sketch configuration not found.\n";
  exit(1);
}
...
cht_config_t get_cht_config(const BDD::BDD &bdd, obj_addr_t cht_addr) {
  ...
  for (auto init_node : init_nodes) {
    ...
    auto capacity = call.args[symbex::FN_CHT_ARG_CAPACITY].expr;
    auto height = call.args[symbex::FN_CHT_ARG_HEIGHT].expr;
    auto cht = call.args[symbex::FN_CHT_ARG_CHT].expr;
  ...
  Log::err() << "CHT configuration not found.\n";
  exit(1);
}
