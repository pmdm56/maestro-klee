...
Constant_ptr const_to_ast_expr(const klee::ref<klee::Expr> &e) {
  ...
  if (expr->getKind() != klee::Expr::Kind::Constant && e->getWidth() <= 64) {
    auto value = util::solver_toolbox.value_from_expr(expr);
    auto value_expr =
        util::solver_toolbox.exprBuilder->Constant(value, expr->getWidth());
    auto always_eq =
        util::solver_toolbox.are_exprs_always_equal(expr, value_expr);
  ...
  if (type->get_type_kind() == Type::TypeKind::ARRAY) {
    ...
    for (unsigned int offset = 0; offset < array->get_n_elems(); offset++) {
      auto byte =
          util::solver_toolbox.exprBuilder->Extract(constant, offset * 8, 8);
      auto value = util::solver_toolbox.value_from_expr(byte);
...
std::vector<Expr_ptr> apply_changes(AST *ast, Expr_ptr variable,
  ...
  if (util::solver_toolbox.are_exprs_always_equal(before, after)) {
    return changes;
  }
...
std::vector<Expr_ptr> build_and_fill_byte_array(AST *ast, Expr_ptr var,
  ...
  for (unsigned int byte = 0; byte < bit_size / 8; byte++) {
    ...
    auto extracted = util::solver_toolbox.exprBuilder->Extract(expr, byte * 8,
                                                              klee::Expr::Int8);
    Expr_ptr val_read = transpile(ast, extracted);
...
apply_changes_to_match(AST *ast, const klee::ref<klee::Expr> &before,
  ...
  case Type::TypeKind::STRUCT: {
    ...
    for (auto field : fields) {
      ...
      auto e1_chunk =
          util::solver_toolbox.exprBuilder->Extract(before, offset, field_size);
      auto e2_chunk =
          util::solver_toolbox.exprBuilder->Extract(after, offset, field_size);

      bool eq = util::solver_toolbox.are_exprs_always_equal(e1_chunk, e2_chunk);
  ...
  case Type::TypeKind::ARRAY: {
    ...
    for (unsigned int offset = 0; offset < a->get_size(); offset += elem_sz) {
      auto e1_chunk =
          util::solver_toolbox.exprBuilder->Extract(before, offset, elem_sz);
      auto e2_chunk =
          util::solver_toolbox.exprBuilder->Extract(after, offset, elem_sz);
  ...
  if (var == nullptr) {
    ...
    if (var == nullptr) {
      ...
      std::cerr << "Variable with symbol '" << symbol << "' not found:"
                << "\n";
      std::cerr << util::expr_to_string(eref) << "\n";
      std::cerr << "\n";
